# COMP 409X: Data Management and Architectures for Data Science
**Detailed Syllabus & Lesson Plan**

**Prerequisite:** COMP 4097 (Database Systems)
...
*   **Lesson 8: DML, Basic Querying & Transactions**
    *   **Topics:** `INSERT`, `UPDATE`, `DELETE`. Basic `SELECT` with `WHERE`, `ORDER BY`. **Transaction Control:** `BEGIN`, `COMMIT`, `ROLLBACK`. Transactions as an integrity mechanism for data ingestion.
    *   **Lab (Colab):** Populate tables, run basic CRUD operations, and experiment with transaction rollbacks.
...
*   **Lesson 19: MongoDB Essentials**
    *   **Topics:** MongoDB Atlas setup. Basic CRUD (`find`, `insert`, `update`, `delete`).
    *   **Lab (Colab):** Connecting to Atlas and performing basic operations.
*   **Lesson 20: Advanced Document Querying**
    *   **Topics:** Querying arrays and nested structures. **MongoDB Aggregation Pipeline** (`$match`, `$group`, `$project`, `$unwind`).
    *   **Lab (Colab):** Analyzing a hierarchical dataset (e.g., product logs) using aggregation stages.

---

## Module 4: Security and Data Access
*Focus: Bridging the gap between the database and application code securely.*

### Week 11: Key-Value Stores & Security
*   **Lesson 21: Redis Concepts & Practice**
    *   **Topics:** Key-Value stores. Caching strategies. Fast lookups. Redis data structures.
    *   **Lab (Colab):** Implementing a simple cache or lookup system.
*   **Lesson 22: Data Security & Connectivity**
    *   **Topics:** **SQL Injection** as an attack vector (OWASP Top 10). **Parameterized Queries** as defense. Secure credential management: Environment variables, `.env` files, connection strings.
    *   **Activity:** "Hack the Lab" - Demonstrating a simple SQL injection and fixing it with parameters.

### Week 12: The Data Access Layer (DAL) & Patterns
*   **Lesson 23: Architecture & Connectivity**
    *   **Topics:** Separation of Concerns. The Data Access Layer (DAL). Database Independence. Connection Management: **Connection Pooling**, drivers, and connection strings.
    *   **Activity:** Discussion: Why separation of concerns matters for maintainability.
*   **Lesson 24: Implementation Patterns (SQLAlchemy)**
    *   **Topics:** **DAO** (Data Access Object) Pattern. **Repository Pattern**. **ORM** (Object-Relational Mapping) with **SQLAlchemy**.
    *   **Demo (Python):** A script comparing "Raw SQL" vs. "SQLAlchemy ORM/DAO" approaches.
...
*   **Lesson 29: RAG Integration & Tuning**
    *   **Topics:** Connecting the retrieval layer to the LLM. **Result Tuning & Optimization** (Top-K, similarity thresholds).
    *   **Lab (Colab):** *Final Project Part 2:* Completing and tuning the "Chat with your Data" system.
*   **Lesson 30: Final Project Showcase & Evaluation**
    *   **Activity:** Presentation of Final Projects. *Note: 3 hours of the semester are reserved for in-class evaluations as per the official syllabus.*